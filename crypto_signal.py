...
def send_telegram(message: str):
    ...
    # Allow multiple recipients: "11111,22222" -> sends to each one.
    ...
    # Only hard-fail if EVERY recipient failed.
...
def interpret_rsi(rsi_val):
    if rsi_val is None:
        return "نامشخص"
    if rsi_val <= RSI_OVERSOLD:
        return f"{rsi_val:.0f} — اشباع فروش (فشار فروش شدید) 🔵"
    if rsi_val < 45:
        return f"{rsi_val:.0f} — متمایل به پایین"
    if rsi_val <= 55:
        return f"{rsi_val:.0f} — متعادل"
    if rsi_val < RSI_OVERBOUGHT:
        return f"{rsi_val:.0f} — متمایل به بالا (نزدیک اشباع خرید)"
    return f"{rsi_val:.0f} — اشباع خرید (احتمال اصلاح) 🔴"


def interpret_macd(cross):
    return {
        "up": "کراس صعودی تازه زده 📈",
        "down": "کراس نزولی تازه زده 📉",
        "none": "کراس تازه‌ای نزده",
    }.get(cross, "نامشخص")


def interpret_ma(price, ma50, ma200):
    parts = []
    if ma50 is not None:
        rel = "بالای" if price > ma50 else "زیر"
        parts.append(f"قیمت {rel} MA50 ({fmt_usd(ma50)})")
    if ma50 is not None and ma200 is not None:
        trend = "صعودی" if ma50 > ma200 else "نزولی"
        parts.append(f"روند بلندمدت {trend} (MA200: {fmt_usd(ma200)})")
    return " | ".join(parts) if parts else "داده‌ی کافی نیست"


def format_tf_block(tf):
    if not tf["ok"]:
        return f"🕐 <b>تایم‌فریم {tf['label']}</b>\n⛔️ {tf['reason']}"
    return "\n".join([
        f"🕐 <b>تایم‌فریم {tf['label']}</b>",
        f"▪️ RSI: {interpret_rsi(tf['rsi'])}",
        f"▪️ MACD: {interpret_macd(tf['macd_cross'])}",
        f"▪️ {interpret_ma(tf['price'], tf['ma50'], tf['ma200'])}",
        f"📌 <b>جمع‌بندی: {tf['verdict']}</b> (امتیاز {tf['score']:+d})",
    ])
...
def main():
    ...
    change = current.get("usd_24h_change", 0)
    change_emoji = "🟢" if change >= 0 else "🔴"

    try:
        from zoneinfo import ZoneInfo
        from datetime import datetime
        now_txt = datetime.now(ZoneInfo("Asia/Tehran")).strftime("%H:%M")
        time_line = f"🕰 ساعت گزارش: {now_txt} (به وقت تهران)"
    except Exception:
        time_line = None

    sep = "➖➖➖➖➖➖➖➖"
    lines = [
        "🟠 <b>گزارش بیت‌کوین (BTC)</b>",
        f"💰 قیمت: <b>${current['usd']:,.0f}</b>  {change_emoji} {change:+.2f}٪ در ۲۴ ساعت",
    ]
    if time_line:
        lines.append(time_line)
    lines.append(sep)
    for tf in tf_results:
        lines.append(format_tf_block(tf))
        lines.append(sep)
    lines.append("📰 <b>خلاصه‌ی اخبار</b>")
    if news_summary:
        lines.append(html.escape(news_summary))  # never let Gemini break Telegram HTML
    else:
        lines.append("خبر مهم یا تحلیلی در دسترس نبود.")
    lines.append(sep)
    lines.append("<i>⚠️ این گزارش فقط کمک‌تصمیم است؛ تصمیم نهایی با خودتان.</i>")
